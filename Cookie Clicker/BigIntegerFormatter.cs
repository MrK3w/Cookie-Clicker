...
namespace Cookie_Clicker
{
    public class BigIntegerFormatter
    {
        ...
        private static string FormatNumberWithSuffixString(string numberAsString)
        {
            ...
            int exponentIndex = numberAsString.Length - 1;
            ...
            string numberWithoutSuffix = $"{leadingDigit}.{decimals}";
            numberWithoutSuffix = numberWithoutSuffix.TrimEnd('0').TrimEnd('.');

            string suffix = GetSuffixForNumber(exponentIndex / 3);
        ...
        {
            List<string> suffixesList = new List<string>
            {
                "", "k", "M", "B", "Q"
            };
